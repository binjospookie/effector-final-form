...
- [api](docs/api#api)
- [$state](docs/api#formstate)

```ts
const { api, $state } = createForm<{ firstName: string }>({
  onSubmit: async (f) => {
    await sleep(1000);
...
Registers a new field. Returns a field = $state (field state) and api for thid field.
...
$state.getState(); // { active: null, errors: {}, isValidationPaused: false }
```
