...
```ts
const { $state, api } = createForm<{ firstName: string }>({
  onSubmit: onSubmitMock,
  subscribeOn: ['initialValues', 'values'],
});
```

## form.$state no `undefined` values

Values of some _FormState_ properties are forcibly replaced by null to avoid a situation where the derived from `form.$state` may contain _undefined_.
...
// effector-final-form
form.$state.map((s) => s.isValidationPaused); // Store<boolean>
```
...
// effector-final-form
const formState = form.$state.getState();
```
...
// effector-final-form
sample({ clock: form.$state, target });
```
